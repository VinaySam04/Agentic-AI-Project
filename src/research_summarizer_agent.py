...
llm = ChatGoogleGenerativeAI(
    ...
    temperature=0.3,
    max_output_tokens=4096,  # Limits the maximum number of tokens in the model's response
)
...
class ResearchAgentSystem:
    ...
    def planner_agent(self):
        ...
        result = call_llm(
            prompt,
            f"Research Question: {self.state['question']}"
        )
        ...
        self.log_decision(
            f"{len(self.state['sub_questions'])} sub-questions created."
        )

        print("\nSub Questions:")

        for i, question in enumerate(self.state["sub_questions"], start=1):
            print(f"{i}. {question}")
    ...
    def researcher_agent(self):
        ...
        result = call_llm(
            prompt,
            json.dumps({
                "sub_questions": self.state["sub_questions"]
            }),
        )
        ...
        self.log_state(
            f"Research completed for {len(self.state['answers'])} questions."
        )
    ...
    def evaluator_agent(self):
        ...
        self.log_decision(
            f"Overall Score: {evaluation['overall']}"
        )
    ...
    def refine_summary(self):
        ...
        self.log_state(
            "Shared state updated with evaluator feedback."
        )
    ...
    def run(self):
        ...
        while self.state["iteration"] <= self.state["max_iterations"]:
            ...
            if verdict == "pass":
                ...
                self.log_decision(
                    "Evaluation passed. Workflow complete."
                )

                break

            self.log_decision(
                "Evaluation failed. Starting refinement..."
            )
...
def main():
    ...
    system = ResearchAgentSystem(question)
    system.run()
